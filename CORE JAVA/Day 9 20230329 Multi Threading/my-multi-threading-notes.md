...
c) Communication between threads is faster because they share memory, though synchronization overhead exists.
...
class TestShared {
    public static void main(String args[]) {
        SharedMemory ob = new SharedMemory();
        Thread t1 = new Thread(ob, "Thread-A");
        Thread t2 = new Thread(ob, "Thread-B");
        t1.start();
        t2.start();
    }
}
