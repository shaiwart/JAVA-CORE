...
Their only purpose is to serve user defined thread/s. When there is no user thread alive, Daemon thread will die.
...
- **System.gc():** Sends a request to the JVM to run garbage collection. It's not guaranteed to run right away.
- **Runtime.getRuntime().gc():** Another way to request GC, works similarly to System.gc().
- **JVM Flag (-XX:+DisableExplicitGC):** If this option is enabled, calling System.gc() or Runtime.gc() will be ignored. This is sometimes used in production to prevent unnecessary GC calls.
